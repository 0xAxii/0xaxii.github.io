...
소수 $p$에 대한 유한체는 다음처럼 쓴다.
$$
\mathbb{F}_p = \{0,1,2,\dots,p-1\}
$$
곡선 위의 좌표 계산은 $\bmod p$로 진행한다.
일반적인 타원곡선은 다음과 같다.
$$
E: y^2 \equiv x^3 + ax + b \pmod p
$$
곡선 위의 점들은 다음 조건을 만족해야 한다.
$$
(x,y)\in\mathbb{F}_p^2
$$
여기에 항등원인 무한원점 $O$를 추가한다.
...
개인키 $d$로 공개키 $Q$를 계산하는 것은 쉽다.

하지만 공개키 $Q$와 기준점 $G$만 보고 $d$를 찾는 것은 어렵다.
...
R = kG
$$
그리고 $R$을 좌표로 쓰면 다음과 같다.
$$
R = (x_R, y_R)
$$
서명의 첫 번째 값 $r$은 $x_R$에서 만든다.
$$
r \equiv x_R \pmod n
$$
...
서명의 두 번째 값 $s$는 다음처럼 계산한다.
$$
s \equiv k^{-1}(z + rd) \pmod n
$$
만약 $s = 0$이면 다른 $k$를 골라 다시 진행한다.

최종 서명은 $(r,s)$이다.

---

검증자는 메시지 $m$, 서명 $(r,s)$, 공개키 $Q$를 가진다.

개인키 $d$는 모른다.

먼저 서명값이 유효한 범위 안에 있는지 확인한다.
$$
1\le r, s \le n-1
$$
범위를 벗어나면 거절한다.

그 다음 메시지 해시와 $s$의 역원을 계산한다.
$$
\begin{aligned}
z &= \operatorname{int}(H(m)) \\
w &\equiv s^{-1} \pmod n
...
검증 과정에서 $w = s^{-1}$라고 잡았으므로 다음처럼 쓸 수 있다.
$$
...
공개키가 $Q = dG$이므로 위 식은 다음처럼 바뀐다.
$$
kG = u_1G + u_2Q
$$

즉 검증자가 계산한 점은 다음과 같다.
$$
X = u_1G + u_2Q = kG = R
$$

결론적으로 서명자가 만든 $r$과 검증자가 계산한 $X$의 $x$좌표가 같아야 한다.
$$
...
따라서 $k$는 다음처럼 구할 수 있다.
$$
k \equiv \frac{z_1-z_2}{s_1-s_2} \pmod n
$$

모듈러 나눗셈을 역원 곱셈으로 쓰면 다음과 같다.
$$
k \equiv (z_1-z_2)(s_1-s_2)^{-1} \pmod n
$$

$k$를 구하면 개인키도 복구할 수 있다.
$$
...
위의 설명대로라면 서명은 $(r, s)$로 검증할 수 있다.
그러면 Ethereum 서명에서 나오는 $v$는 뭘까?
Ethereum에서는 `ecrecover(z, v, r, s)`와 같은 함수를 쓴다. `ecrecover`는 서명이 유효하면 서명자의 주소를, 실패하면 `address(0)`을 반환하는 함수이다.

이더리움 주소는 공개키에서 만들어진다.
공개키를 $Q=(x_Q,y_Q)$라 하면, 대략 다음과 같이 주소가 정해진다.
$$
hash = keccak256(x_Q || y_Q)
$$

이 `hash`의 마지막 20바이트가 주소가 된다.
따라서 `ecrecover`가 서명으로부터 공개키 $Q$를 복구할 수 있다면, 그 공개키에 대응되는 주소도 구할 수 있다.

문제는 서명에 들어있는 $r$이 원래 서명 과정에서 만든 점 $R=kG$의 $x$좌표에서 나온다는 점이다.

타원곡선에서는 같은 $x$값을 가진 점이 보통 $(x, y)$, $(x, -y)$ 두 개가 있다.

즉 $r$만으로는 둘 중 어느 점이 실제 $R$인지 알 수 없다.

이때 어느 쪽 점을 써야 하는지 알려주는 recovery id가 $v$이다.
이 문제에서는 일반적인 Ethereum 서명처럼 $v$가 27 또는 28로 들어온다.
...
이제 이벤트에서 얻은 서명이 있다고 하자.
$$
(r,s,v)
$$

위에서 봤듯이 원본 서명을 그대로 재사용하는 것도 가능하다.
여기서는 ECDSA malleability를 이용해 다른 유효 서명을 만들어보자.

서명 검증에서 계산되는 점은 결국 $R$의 $x$좌표와 관련된다.

그런데 $R$과 $-R$은 $y$좌표만 반대이고 $x$좌표는 같다.

따라서 $R$ 대신 $-R$을 만들 수 있으면 같은 $r$로도 검증을 통과할 수 있다.
...
$v$는 27과 28을 사용하는데, 둘 중 어느 $y$좌표를 가진 $R$을 쓸지 나타내는 값이다.

$R$을 $-R$로 바꿨으니 $v$도 반대 값으로 바꿔야 한다.
...
따라서 원본 서명 대신 다음 값을 사용한다.
$$
(r,n-s,55-v)
$$

이 값은 원본과 다른 서명이지만 같은 controller 주소로 복구된다.

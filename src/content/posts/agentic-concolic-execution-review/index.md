...
CONCOLLMIC은 LLM을 솔버 자리에 그대로 놓지 않는다.
...
논문 메타 리뷰에서도 평가 프로그램 규모가 제한적이고, 높은 수준의 의미 해석과 낮은 수준의 정밀한 실행 사이의 균형을 충분히 보여주지 못했다는 지적이 있었다. LLM 출력이 매번 같지 않기 때문에 재현성도 걱정거리다. 가능성은 크지만, 아직 안정적인 산업용 도구라고 보기는 어렵다.

...
이 논문은 기존 concolic execution이 실제 프로그램에서 막히는 이유를 찾는 것에서 시작한다. 라이브러리나 실행 환경을 전부 심볼릭 모델로 만들기 어렵고, 그렇게 만든 조건도 solver가 끝까지 풀기 어렵다는 문제다.

CONCOLLMIC은 이 문제를 LLM에게 "버그 찾아줘"라고 맡기는 방식으로 풀지 않는다. 실행 기록을 읽게 하고, 다음에 열어볼 분기를 고르게 한 뒤, 그 분기에 필요한 입력과 환경 설정을 만들게 한다. 그리고 그 결과를 실제 실행으로 확인한다.

## 1. 논문을 고른 이유

이 논문을 고른 이유는 LLM을 보안 분석에 쓰는 방식이 단순한 코드 요약이나 취약점 질의가 아니었기 때문이다.

요즘 내가 관심 있는 하네스 엔지니어링도 이러한 방식과 닮아 있다. 하네스란 LLM이 특정 작업을 잘 수행하도록 프롬프트, 도구 호출, 실행 환경, 피드백, 검증 루프를 묶어 둔 작업 틀을 뜻한다. Google OSS-Fuzz에서도 Fuzz Introspector로 덜 퍼징된 함수를 찾고, LLM이 퍼즈 타깃을 만들고, 빌드/런타임 피드백으로 고치는 흐름을 보여준다. 내가 관심 있게 보는 지점도 결국 LLM을 혼자 두지 않고 성능을 끌어올리기 위해 작업 환경을 설계한다는 점이다.

이 논문도 같은 문제를 공유한다고 생각한다. 좋은 입력 하나를 찍어내는 것보다, LLM이 실행 기록을 보고 도구를 호출하고 실패를 통해 피드백받게 만드는 구조가 더 중요할 수 있다는 점이다.
...
정리하면 기존 방식과 CONCOLLMIC의 차이는 이렇다.

| 기존 concolic execution | CONCOLLMIC |
| --- | --- |
| 실행 경로를 낮은 수준의 제약식으로 모은다. | 실행 로그를 읽고 필요한 조건을 더 높은 수준으로 다시 쓴다. |
| 라이브러리, 런타임, 환경 동작을 도구 안에 직접 모델링해야 한다. | 프로그램을 실제로 실행하고, 입력과 환경을 바꿔가며 확인한다. |
| solver가 풀기 좋은 식을 만드는 것이 중요하다. | 자연어, Python, Z3 식 중 맞는 표현을 고르고 도구를 조합한다. |
| 새 입력이 의미 있는지는 도구의 내부 모델에 많이 기대게 된다. | 마지막에 실제 실행으로 목표 분기에 도달했는지 검증한다. |

내가 보기엔 이 논문의 출발점은 "LLM이 더 똑똑한 솔버인가?"가 아니라 오히려 "사람이 코드 의미를 읽고 조건을 줄여 쓰듯이, LLM이 조건을 더 다루기 쉬운 형태로 바꿀 수 있는가?"가 맞다. 더 다루기 쉬운 형태라는 것은, 기존 방식과 같은 수식일 수도, 자연어일 수도 있다.
...
이 논문만의 독특한 접근이 여기에 있다. 조건을 꼭 SMT 식으로만 쓰지 않는다. 어떤 조건은 자연어가 더 짧고 정확할 수 있고, 어떤 조건은 Python 코드로 계산하는 게 낫고, 어떤 조건은 Z3에 맡기는 게 낫다. FP-Bench 예시처럼 "두 부동소수점 값 사이에 표현 가능한 값이 20개 이하"라는 조건은 사람이 보기에는 한 문장인데, 낮은 수준의 식으로 풀면 `atof`, 반복문, 부동소수점 표현까지 줄줄이 따라온다. CONCOLLMIC은 이런 부분을 더 높은 수준의 조건으로 접는다.
...
| 평가 대상 | 비교 조건 | 결과 |
| --- | --- | --- |
| 8개 C/C++ 프로그램 | 각 도구를 대상마다 다섯 번 실행하고, GCov 기준 분기 커버리지를 비교 | KLEE 대비 233%, KLEE-Pending 대비 135%, SymCC 대비 130%, SymSan 대비 115% 더 높은 분기 커버리지 |
| AFL++ 비교 | 48시간 AFL++ 실행 결과와 비교 | 평균 81% 높은 커버리지 |
| 다중 언어 프로그램 | 기존 다중 언어 심볼릭 실행 도구가 마땅히 없어, 초기 입력 대비 CONCOLLMIC 내부 라인 커버리지 증가를 측정 | ultrajson 3.5배, jansi 8.2배, py4j 1.9배, protobuf-go 1.9배 커버리지 증가 |
| FP-Bench | 부동소수점 추론용 벤치마크 26개에서 KLEE-Float, 일반 KLEE와 비교 | KLEE-Float보다 20%, 일반 KLEE보다 107% 높은 커버리지 |
| 버그 탐지 | 실제 프로그램과 다중 언어 프로그램에서 새로 도달한 경로를 확인 | 11개 신규 버그 발견, 9개는 확인 또는 수정, libsoup 버그는 CVE-2025-4945 |

실험은 도커 컨테이너에서 2 CPU, 8GiB RAM 조건으로 진행됐고, 각 도구를 대상마다 다섯 번 실행했다. 이 표에서 중요한 건 수치마다 비교 기준이 조금씩 다르다는 점이다. C/C++ 프로그램에서는 기존 DSE 도구들과 같은 대상의 분기 커버리지를 비교했고, AFL++와는 48시간 퍼징 결과를 기준으로 비교했다. 다중 언어 프로그램은 비교 가능한 기존 도구가 부족해서 초기 입력 대비 얼마나 더 많은 코드를 실행했는지를 봤다.

숫자만 보면 CONCOLLMIC이 KLEE나 AFL++보다 낫다는 이야기로 끝나기 쉬워 보인다. 그런데 그렇게만 보면 오해가 생길 수 있다. KLEE류 도구는 잘 정의된 심볼릭 모델과 솔버가 강한 영역에서 장점이 있다. 반대로 CONCOLLMIC은 라이브러리 호출, 환경 변수, CLI 인자, 네트워크 입력처럼 모델링하기 까다로운 영역에서 강점을 보인다.
...
읽고 나서 가장 크게 기억에 남은 건 LLM이 작업을 하게 만드는 틀을 설계하는 방식이다.
...
이 생각은 Web3 버그바운티 쪽 하네스 엔지니어링 설계 고민으로 이어졌다. 스마트컨트랙트 분석에서도 LLM에게 바로 취약점을 물어보는 것보다, 목표 상태를 정하고, 필요한 컨텍스트와 도구를 주고, 실행 결과를 다시 피드백하는 작업 틀을 만드는 쪽이 더 가능성이 있어 보였다.

## 9. 앞으로 해볼 것

여기부터는 논문에서 직접 다룬 내용이 아니라, 내가 따로 확장해서 생각한 실험 아이디어다.

위의 Web3 버그바운티 쪽 하네스 엔지니어링 설계를 하자면, 바로 처음부터 전체 DeFi 프로토콜을 대상으로 삼기보다 하나의 공개된 버그바운티 스코프 안에서 "특정 상태가 되어야만 실행되는 분기"를 하나 고르는 게 좋을 것 같다.
...
CONCOLLMIC은 concolic execution을 LLM으로 단순히 다시 포장한 논문이라기보다, 기존 도구가 막히는 지점에 LLM 에이전트를 끼워 넣어서 해결하려는 연구에 가깝다.

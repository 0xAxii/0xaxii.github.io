...
- 브랜치: 특정 커밋을 가리키는 포인터
- HEAD: 현재 작업 중인 위치
- 원격 추적 브랜치: 원격 브랜치의 상태를 로컬에서 기억하는 포인터
...
`merge`는 다른 브랜치의 변경사항을 현재 브랜치에 합친다. 단, fast-forward가 가능하면 병합 커밋 없이 브랜치 포인터만 이동하고, `--no-ff`를 쓰면 병합 커밋을 강제로 남긴다. 위 그림의 merge는 fast-forward가 아닌 경우다.

`rebase`는 작업 커밋을 새 기준 브랜치 뒤에 다시 쌓아 한 줄처럼 정리한다.
...
- `git rebase <newbase>` 현재 브랜치의 커밋을 새 기준 위로 다시 쌓기
- `git rebase <newbase> <branch>` 지정한 브랜치를 새 기준 위로 재배치
...
`cherry-pick`은 작업 트리가 깨끗한 상태에서 실행하는 편이 안전하다. 충돌이 나면 충돌을 해결한 뒤 계속 진행해야 한다.
...
자주 쓰는 선택지는 `pick`, `reword`, `edit`, `squash`, `fixup`, `drop`이다. 커밋을 남길지, 메시지를 바꿀지, 합칠지, 제거할지를 여기서 정한다.

## 6. 표시와 임시 보관

태그는 중요한 커밋에 붙이는 고정 이름표다. stash는 아직 커밋하기 애매한 변경사항을 잠깐 치워두는 용도다.

- `git tag <tag>` 현재 HEAD에 lightweight tag 추가
- `git tag <tag> <commit>` 지정한 커밋에 lightweight tag 추가
- `git tag -a <tag> -m "message"` annotated tag 생성
- `git push origin <tag>` 특정 태그를 원격에 업로드
- `git push origin --tags` 로컬의 모든 태그를 원격에 업로드
- `git describe` 가까운 annotated tag 기준으로 현재 위치 설명
- `git describe --tags` lightweight tag까지 포함해 가까운 태그 기준으로 설명

![](/assets/notion/git-stash-ai-v3.png)

`stash`는 작업 중인 변경사항을 임시로 보관했다가, 나중에 다시 꺼내 적용할 수 있게 해준다.

- `git stash` 추적 중인 파일의 작업 디렉터리/스테이징 변경사항 임시 저장
- `git stash -u` untracked 파일까지 포함해 임시 저장
- `git stash -a` ignored 파일까지 포함해 임시 저장
- `git stash list` stash 목록 확인
- `git stash pop` 저장한 변경사항을 다시 적용하고 stash 목록에서 제거 시도
...
`fetch`는 원격 상태를 원격 추적 브랜치에 가져오기만 한다. `pull`은 가져온 원격 변경사항을 현재 브랜치에 통합하고, `push`는 로컬 커밋을 원격 저장소로 올린다.

- `git clone <url>` 원격 저장소를 로컬로 복제
- `git fetch` 원격 변경사항을 가져오되 현재 브랜치에는 합치지 않음
- `git pull` fetch 후 가져온 원격 변경사항을 현재 브랜치에 통합
- `git pull --rebase` fetch 후 merge 대신 rebase로 통합
- `git pull --no-rebase` fetch 후 merge 방식으로 통합
- `git pull --ff-only` fast-forward가 가능할 때만 통합
- `git push` 로컬 커밋을 원격에 업로드
- `git push -u <remote> <branch>` push하면서 upstream 추적 관계 설정

원격 추적 브랜치는 보통 `origin/main`처럼 표시된다. 실제 원격 브랜치는 원격 저장소에 있고, 로컬의 `origin/main`은 마지막 통신 시점의 원격 상태를 기억한다. 학습 환경에서는 `o/main`처럼 짧게 보이기도 한다.
...
- `git pull <remote> <source>:<destination>` 가져온 뒤 현재 브랜치에 통합
...
아직 원격 `main`에 push하지 않은 상태이고, 커밋되지 않은 작업 변경이 없다면 다음처럼 새 브랜치를 먼저 만든 뒤 `main`을 원격 상태로 되돌릴 수 있다. `--hard`는 작업 디렉터리 변경까지 되돌릴 수 있으므로 먼저 `git status`를 확인해야 한다.
...
이미 원격 `main`에 push한 커밋이라면 이 흐름보다 `revert`를 쓰거나 팀과 합의한 뒤 별도로 처리하는 편이 안전하다.

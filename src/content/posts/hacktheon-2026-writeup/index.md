...
일반 TTS로 target sentence를 합성하면 text similarity는 충분했다.
speaker similarity는 낮다.
reference wav를 그대로 넣으면 speaker similarity는 높다.
대신 transcript가 달라 text similarity가 낮다.

그래서 reference 음성을 ASR에는 의미 없는 소리처럼 만들고, speaker embedding에는 화자 특징을 남겼다.
reference wav를 reverse한 뒤 낮은 볼륨으로 target TTS 뒤에 붙였다.

앞부분의 TTS 때문에 ASR은 목표 문장을 주로 인식한다.
뒤쪽의 reverse reference는 transcript에는 크게 섞이지 않는다.
대신 speaker verification에는 화자 특징을 보탠다.
